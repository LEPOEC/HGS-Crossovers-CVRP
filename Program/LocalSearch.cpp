...
std::tuple<int,int,LSStats> LocalSearch::run(Individual & indiv, double penaltyCapacityLS, double penaltyDurationLS)
{
	moveStats = LSStats{};
	...
	return {nbMoves,loopID,moveStats};
}
...
void LocalSearch::exportIndividual(Individual & indiv)
{
	...
	for (int r = 0; r < params.nbVehicles; r++)
		routePolarAngles.push_back({routes[r].polarAngleBarycenter, r});

	if (params.ap.chromTOrder == 0)
		std::sort(routePolarAngles.begin(), routePolarAngles.end());
	else
		std::shuffle(routePolarAngles.begin(), routePolarAngles.end(), params.ran);

	int pos = 0;
	for (int r = 0; r < params.nbVehicles; r++)
	{
		indiv.chromR[r].clear();
		int routeIdx = routePolarAngles[r].second;

		if (params.ap.chromTOrder <= 1) {
			Node * node = depots[routeIdx].next;
			while (!node->isDepot) {
			...
			// Mode 2 : shuffle routes + renversement aléatoire par route
			std::vector<int> routeClients;
			Node * node = depots[routeIdx].next;
			while (!node->isDepot) {
				routeClients.push_back(node->cour);
				node = node->next;
			}
			if (params.ran() % 2 == 0)
				std::reverse(routeClients.begin(), routeClients.end());
			for (int c : routeClients) {
				indiv.chromT[pos] = c;
				indiv.chromR[r].push_back(c);
				pos++;
			}
		}
	}
